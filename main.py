...
import torchio
from models.vgg16 import vgg3d, ADnetwork
...
def train(load=False, target='score', lr=opt.lr, img_type=opt.img_type, transform=False):
    ...
    :param target: 'score': use the original attention score(scale:50~100) to train;
                   ...
                   target must be 'score_group' when classification mode
    ...
    global_step = 0
    if not load:
        model = vgg3d(classify=False).cuda()
    else:
        model = torch.load('./checkpoints/exp_score_09-08.pth').cuda()
...
def val(model, dataloader):
    ...
    with torch.no_grad():
        for batch_idx, data in tqdm(enumerate(dataloader)):
            print(data['img']['data'].shape)
            val_input = data['img']['data'].view(-1, 1, 190 - opt.crop_size * 2, 190 - opt.crop_size * 2,
                                                 190 - opt.crop_size * 2)
...
def train_class(load=False, target='score_group', lr=opt.lr, img_type=opt.img_type, transform=False):
    '''
    :param load: Load pre-trained model if True
    :param target: 'score': use the original attention score(scale:50~100) to train;
                   'score_center': use centered score(score-mean_score) to train;
    ...
    global_step = 0
    if not load:
        model = vgg3d(classify=True).cuda()
    else:
        model = torch.load('./checkpoints/exp_fc_last.bias_09-06.pth').cuda()

    # loss & optimizer
    criterion = torch.nn.BCEWithLogitsLoss()  # only tried MAE here, you can customize the loss or use other choices
    ...
    for epoch in range(opt.max_epoch):
        ...
        for batch_idx, data in enumerate(training_loader):
            ...
            # get labels
            targets = torch.tensor(list(map(int, data['target'])), dtype=torch.long)   # dtype: int
            ...
            y[torch.tensor(range(y.shape[0]), dtype=torch.long), targets] = 1  # one-hot label
            y = y.cuda()

            loss = criterion(res[0], y)
            print("train_loss: {}".format(loss.data))
            ...
            # validation
        val_loss, val_running_loss = val_class(model, val_loader)  # test for each iter
        writer.add_scalar('val loss',
                            val_loss,
                            epoch )

        # save model for every epoch
        torch.save(model, './checkpoints/exp_class_{}_{}.pth'.format(target, time.strftime('%m-%d')))
...
def val_class(model, dataloader):
    """
    validation
    """
    # settings
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in tqdm(enumerate(dataloader)):
            ...
            val_input = data['img']['data'].view(-1, 1, 190 - opt.crop_size * 2, 190 - opt.crop_size * 2,
                                                 190 - opt.crop_size * 2)
            print("val input size: {}".format(val_input.shape))
            val_label = torch.tensor(list(map(int, data['target'])), dtype=torch.long)
            ...
            val_res = model(val_input)
            _, predicted = torch.max(val_res[0], 1)
            val_loss = criterion(val_res[0], y)  # predicted, label
            ...
            print("val avg loss: {}".format(val_loss_meter))
